...
def find_data_row(all_data, direction, date_col, target_date):
    ...
    for row in ordered:
        ...
        if row[date_col].strip() in variants: return row, row[date_col].strip(), True
    for row in ordered:
        if not valid(row): continue
        v = row[date_col].strip()
        if any(var in v for var in variants): return row, v, True
    for row in ordered:
        if not valid(row): continue
        if sum(1 for c in row if c.strip()) > 3: return row, row[date_col].strip(), False
...
def fetch(dept, yesterday):
    ...
    try:
        ...
        if not all_data: return f"【{label}】⚠️ 空表", None, None
        row, date_str, exact = find_data_row(all_data, dept["direction"], dept["date_col"], yesterday)
        if row is None: return f"【{label}】⚠️ 无有效数据", None, None
        ...
        for name, col in dept["columns"].items():
            ...
            if name in SUMMARY_FIELDS: data[name] = parse_num(val)
            raw[name] = val
        return "\n".join(lines), data, raw
    except Exception as e:
        return f"【{label}】❌ {e}", None, None

def build_summary(label, members, results):
    totals = {f: 0.0 for f in SUMMARY_FIELDS}
    missing = []
    for m in members:
        d = results.get(m)
        if d is None: missing.append(m); continue
        for f in SUMMARY_FIELDS: totals[f] += d.get(f, 0.0)
    lines = [f"【{label}】({'、'.join(members)})"]
    for f in SUMMARY_FIELDS: lines.append(f"  {f}: {fmt_num(totals[f])}")
    if missing: lines.append(f"  ⚠️ 缺失: {' '.join(missing)}")
    return "\n".join(lines)

def save_to_history(yesterday, dept_raw):
    if not HISTORY_SHEET_ID:
        print("[History] 未设置 HISTORY_SHEET_ID，跳过保存"); return
    try:
        ...
        if date_str in existing:
            print(f"[History] {date_str} 已存在，跳过写入"); return
        rows = []
...
def load_history(yesterday):
    if not HISTORY_SHEET_ID: return {}
    try:
        ss = client.open_by_key(HISTORY_SHEET_ID)
        ws = ss.worksheets()[0]
        all_data = ws.get_all_values()
        ...
        for row in all_data[1:]:
            ...
            try: datetime.strptime(row[0], '%Y-%m-%d')
            except: continue
            parsed.append({'date_str':row[0],'dept':row[1] if len(row)>1 else '',
                '注册':row[3] if len(row)>3 else '','首存':row[4] if len(row)>4 else '',
                '存款':row[5] if len(row)>5 else '','提款':row[6] if len(row)>6 else '',
                '存提差':row[7] if len(row)>7 else '','活跃':row[8] if len(row)>8 else ''})
        ...
        def period_totals(start, end):
            ...
            totals = {}
            for r in parsed:
                if r['date_str'] < s or r['date_str'] > e or not r['dept']: continue
                dept = r['dept']
                if dept not in totals: totals[dept] = {f: 0.0 for f in HISTORY_FIELDS}
                for f in HISTORY_FIELDS: totals[dept][f] += parse_num(r[f])
            return {dept: {f: fmt_num(v) for f, v in fields.items()} for dept, fields in totals.items()}
        ...
        return {'前日数据': day_data(day_before), '7天前数据': day_data(week_ago),
                '本月历史累计': history_month, '上月同期累计': last_month, '本月天数': yesterday.day}
    except Exception as e:
        print(f"[History Load Error] {e}"); return {}

def build_analysis_payload(yesterday, dept_raw, history):
    mt_members = ["QY","TH","LW","QM","RB"]
    rt_members = ["UED","JX","TQ"]

    def group_sum(members, field, source):
    ...
    if history:
        ...
        if prev:
            ...
            payload["日环比（今日 vs 前日）"] = {
                ...
                "各部门存款": {dept["label"]: pct_change(
                    parse_num((dept_raw.get(dept["label"]) or {}).get('存款','0')),
                    parse_num((prev.get(dept["label"]) or {}).get('存款','0'))) for dept in DEPARTMENTS}
            }
        if w7:
        ...
        if month_hist or last_month:
            def month_total(field, members):
                return group_sum(members, field, month_hist) + group_sum(members, field, dept_raw)
            def last_total(field, members):
                return group_sum(members, field, last_month)
            payload["本月累计（含今日）"] = {
...
（COO 最需要关注的一件事）"""
    try:
        ...
        message = ai_client.messages.create(model=CLAUDE_MODEL, max_tokens=1200,
...
async def main():
    ...
    raw_report = (f"📊 {date_label} 各部门原始数据\n\n"
        + "\n\n".join(blocks)
        + f"\n\n{'─'*20}\n\n" + mt_summary + "\n\n" + rt_summary)
    ...
    if ai_report:
        trend_note = "（含日/周/月趋势对比）" if has_history else "（历史数据积累中，明日起生效）"
        await bot.send_message(chat_id=TG_CHAT_ID, text=f"🤖 {date_label} COO 经营简报 {trend_note}\n\n{ai_report}")
        await bot.send_message(chat_id=TG_CHAT_ID, text=raw_report)
    else:
        await bot.send_message(chat_id=TG_CHAT_ID, text=raw_report)
        if CLAUDE_API_KEY:
            await bot.send_message(chat_id=TG_CHAT_ID, text="⚠️ AI 分析暂时不可用，已发送原始数据日报。")
